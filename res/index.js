...
(function() {
	...
	setFullscreen();
	document.getElementById('jumpbutton2').addEventListener('mousedown', buttonEffect);
	...
	function buttonEffect() {
		...
		for(var i=0;i<200;i=i+1) {
			...
			createParticle({
				...
				y: y,
				degree: degree,
				speed: Math.randMinMax(180, 240),
				vs: Math.randMinMax(-4,-1)
			});
		}
	}
	window.setTimeout(buttonEffect, 200); 
	...
	function createParticle( args ) {
		...
		var options = {
			x: width/2,
			y: height/2,
			color: 'hsla('+ Math.randMinMax(2, 700) +', 100%, 50%, '+(Math.random().toFixed(2))+')',
