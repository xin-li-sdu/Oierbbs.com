<?php if(!defined('IN_DISCUZ')) exit('Access Denied'); hookscriptoutput('spacecp_domain');
0
|| checktplrefresh('./template/default/home/spacecp_domain.htm', './template/default/home/spacecp_profile_nav.htm', 1532756228, '1', './data/template/1_1_home_spacecp_domain.tpl.php', './template/default', 'home/spacecp_domain')
|| checktplrefresh('./template/default/home/spacecp_domain.htm', './template/default/home/spacecp_footer.htm', 1532756228, '1', './data/template/1_1_home_spacecp_domain.tpl.php', './template/default', 'home/spacecp_domain')
;?><?php include template('common/header'); include template('home/spacecp_header'); ?><ul class="tb cl">
...
</ul><form method="post" autocomplete="off" action="home.php?mod=spacecp&amp;ac=domain&amp;op=">
<input type="hidden" name="formhash" value="<?php echo FORMHASH;?>" />
<?php if(!$space['domain'] && $dcredit) { ?><p>修改域名将需要支付积分 <?php echo $consume，您现在有拥有;?> <?php echo $credits;?></p><?php } ?>
<div class="mtw bm bmn hm">
<p class="mtw mbn">
http://<input type="text" name="domain" value="<?php echo $space['domain'];?>" size="20" class="px vm" />.<?php echo $_G['setting']['domain']['root']['home'];?>
<input type="hidden" name="domainsubmit" value="true" />&nbsp;
<button type="submit" name="domainsubmitbtn" id="domainsubmitbtn" value="true" class="pn vm" /><em>修改域名</em></button>
</p>
<p class="mbw">域名可使用最少 <?php echo $domainlength;?> 个 ，最多 30 个的字母或数字组合，且必须字母开头 </p>
</div>
